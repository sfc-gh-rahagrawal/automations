...
-- Deploy: snow dcm deploy --target dev|test|prod

DEFINE SEMANTIC VIEW COLES_RETAIL_DB{{env_suffix}}.AI_ANALYTICS.COLES_RETAIL_SEMANTIC_MODEL
tables (
TIME_DIM as COLES_RETAIL_DB{{env_suffix}}.CORE.DIM_TIME primary key (DATE_KEY) comment='Calendar dimension with daily grain, 5 years (2023-2027). Use for date filtering, fiscal periods, and seasonal trends.',
STORES as COLES_RETAIL_DB{{env_suffix}}.AI_ANALYTICS.V_STORES primary key (LOC_ID) comment='Store dimension with 10 Coles locations. LOC_NM = full store name (e.g. Coles Melbourne CBD). FMT_CDE = format: Metro, Mall, Suburban.',
PRODUCTS as COLES_RETAIL_DB{{env_suffix}}.AI_ANALYTICS.V_PRODUCT_MASTER primary key (MERCH_ID) comment='Product master. PRODUCT_CATEGORY = Dairy, Bakery, Beverages, Cereals, Frozen, Household, Meat, Pantry, Produce. BRAND = supplier/brand.',
SUPPLIERS as COLES_RETAIL_DB{{env_suffix}}.SUPPLY_CHAIN.DIM_SUPPLIERS primary key (SUPPLIER_ID) comment='Supplier dimension with vendor details, country, reliability scores, and organic certification.',
SALES as COLES_RETAIL_DB{{env_suffix}}.RETAIL.FACT_SALES_TRANSACTIONS primary key (TXN_ID) comment='Sales transactions. NET_AMT is AFTER discounts — do NOT subtract DISC_AMT. MARGIN_AMT is gross profit.',
STORE_PERFORMANCE as COLES_RETAIL_DB{{env_suffix}}.RETAIL.FACT_DAILY_STORE_PERFORMANCE primary key (PERFORMANCE_ID) comment='Daily store performance: revenue, transactions, basket size, foot traffic, conversion rate.',
REVIEWS as COLES_RETAIL_DB{{env_suffix}}.MARKETING.FACT_CUSTOMER_REVIEWS primary key (REVIEW_ID) comment='Customer reviews with 1-5 star ratings and free-text feedback. store_location has short names like Melbourne CBD.',
REVIEW_SENTIMENT as COLES_RETAIL_DB{{env_suffix}}.MARKETING.REVIEWS_SENTIMENT_CATEGORIES primary key (REVIEW_ID,SENTIMENT_CATEGORY) comment='AI sentiment per review aspect (product_quality, value, freshness, service, store_cleanliness, parking). Values: positive, negative, neutral, mixed, unknown.',
INVENTORY_ALERTS as COLES_RETAIL_DB{{env_suffix}}.SUPPLY_CHAIN.FACT_INVENTORY_ALERTS primary key (ALERT_ID) comment='Inventory alerts for stock issues across stores.',
SUPPORT_TICKETS as COLES_RETAIL_DB{{env_suffix}}.MARKETING.FACT_SUPPORT_TICKETS primary key (TICKET_ID) comment='Support tickets with AI-classified categories. Priority: CRITICAL, HIGH, MEDIUM, LOW.'
)
relationships (
