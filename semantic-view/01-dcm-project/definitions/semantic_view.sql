...
tables (
TIME_DIM as COLES_RETAIL_DB{{env_suffix}}.AI_ANALYTICS.V_TIME_DIM primary key (DATE_KEY) with synonyms=('date','calendar','time'),
...
SALES as COLES_RETAIL_DB{{env_suffix}}.AI_ANALYTICS.V_SALES_ENRICHED primary key (TXN_ID) with synonyms=('transactions','orders'),
STORE_PERFORMANCE as COLES_RETAIL_DB{{env_suffix}}.RETAIL.FACT_DAILY_STORE_PERFORMANCE primary key (PERFORMANCE_ID),
REVIEWS as COLES_RETAIL_DB{{env_suffix}}.RETAIL.FACT_CUSTOMER_REVIEWS primary key (REVIEW_ID) with synonyms=('feedback','ratings'),
REVIEW_SENTIMENT as COLES_RETAIL_DB{{env_suffix}}.RETAIL.REVIEWS_SENTIMENT_CATEGORIES primary key (REVIEW_ID,SENTIMENT_CATEGORY) with synonyms=('sentiment','customer sentiment','review sentiment'),
